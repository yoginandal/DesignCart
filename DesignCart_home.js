...
///////////////////////text-change(Brand Identity)/////////////////
var word_Branding = ['Vectored logos','Website for Brands','Brand Advertisment','Awesome Designs'],
    parts_Branding,
    m = 0,
    offsets_Branding = 0,
    len_Branding = word_Branding.length,
    forward_Branding = true,
    skip_counts_Branding = 0,
    skip_delays_Branding = 15,
    speeds_Branding = 70;
var wordflicks_Branding = function () {
  setInterval(function () {
    if (forward_Branding) {
      if (offsets_Branding >= word_Branding[m].length) {
        ++skip_counts_Branding;
        if (skip_counts_Branding == skip_delays_Branding) {
          forward_Branding = false;
          skip_counts_Branding = 0;
        }
      }
    }
    else {
      if (offsets_Branding == 0) {
        forward_Branding = true;
        m++;
        offsets_Branding = 0;
        if (m >= len_Branding) {
    ...
    parts_Branding = word_Branding[m].substring(0, offsets_Branding);
    if (skip_counts_Branding == 0) {
      if (forward_Branding) {
        offsets_Branding++;
      }
      else {
    ...
    document.getElementById('Brand_Identity_text_change').innerText = parts_Branding;
  },speeds_Branding);
};
...
//////////////////////text-change(Graphic Design)////////////////////
var word_design = ['Branding & Designing','creative use of images','Staggring use of colors'],
    parts_design,
    l = 0,
    offsets_design = 0,
    len_design = word_design.length,
    forward_design = true,
    skip_counts_design = 0,
    skip_delays_design = 15,
    speeds_design = 70;
var wordflicks_design = function () {
  setInterval(function () {
    if (forward_design) {
      if (offsets_design >= word_design[l].length) {
        ++skip_counts_design;
        if (skip_counts_design == skip_delays_design) {
          forward_design = false;
          skip_counts_design = 0;
        }
      }
    }
    ...
    parts_design = word_design[l].substring(0, offsets_design);
    if (skip_counts_design == 0) {
      if (forward_design) {
        offsets_design++;
      }
      else {
    ...
    document.getElementById('Graphic_Design_text_change').innerText = parts_design;
  },speeds_design);
};
